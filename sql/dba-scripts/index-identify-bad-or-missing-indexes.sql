/**************************************************************************************************************************************************
 Queries below from https://sqlserverperformance.wordpress.com/2007/11/29/useful-tricks-for-finding-missing-and-bad-indexes-in-sql-server-2005/
**************************************************************************************************************************************************/
...
SELECT 
	'Table Name' = object_name(s.object_id), 'Index Name' =i.name, i.index_id
...
-- Missing indexes for a specific db/table
SELECT 
	user_seeks * avg_total_user_cost * (avg_user_impact * 0.01) AS index_advantage
	, migs.last_user_seek
	, mid.statement as 'Database.Schema.Table'
...
INNER JOIN sys.dm_db_missing_index_groups AS mig WITH (NOLOCK) ON migs.group_handle = mig.index_group_handle
INNER JOIN sys.dm_db_missing_index_details AS mid WITH (NOLOCK) ON mig.index_handle = mid.index_handle
WHERE mid.statement LIKE '%DBName%TableName%'
ORDER BY mid.statement;


--if you add any indexes then make sure to update statistics
--EXEC sp_updatestats
